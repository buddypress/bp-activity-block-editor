...
class BP_Activity_Component extends \BP_Activity_Component {
	...
	public function late_includes() {
		...
		if ( bp_is_current_component( 'activity' ) ) {
			...
			if ( is_user_logged_in() ) {
				if ( in_array( bp_current_action(), array( 'delete', 'spam', 'post', 'reply', 'favorite', 'unfavorite' ), true ) ) {
					require_once $this->bp_path . 'bp-activity/actions/' . bp_current_action() . '.php';
				} elseif ( in_array( bp_current_action(), array( 'like', 'dislike' ), true ) ) {
			...
			if ( bp_is_current_action( 'feed' ) || bp_is_action_variable( 'feed', 0 ) ) {
				require_once $this->bp_path . 'bp-activity/actions/feeds.php';
			}

			// Screens - Directory.
			if ( bp_is_activity_directory() ) {
				require_once $this->bp_path . 'bp-activity/screens/directory.php';
			}

			// Screens - User main nav.
			if ( bp_is_user() ) {
				require_once $this->bp_path . 'bp-activity/screens/just-me.php';
			}
			...
			if ( bp_is_user() && isset( $filenames[ $slug ] ) ) {
				require_once $this->bp_path . 'bp-activity/screens/' . $filenames[ $slug ] . '.php';
			}

			if ( bp_is_user() && 'likes' === $slug ) {
				require_once $this->bp_path . 'bp-activity/screens/likes.php';
			}

			// Screens - Single permalink.
			if ( bp_is_current_action( 'p' ) || is_numeric( bp_current_action() ) ) {
				require_once $this->bp_path . 'bp-activity/screens/permalink.php';
			}
	...
	public function register_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		\BP_Component::register_nav( $main_nav, $sub_nav );
	}
	...
	public function setup_admin_bar( $wp_admin_nav = array() ) {
		...
		\BP_Component::setup_admin_bar( $wp_admin_nav );
	}
